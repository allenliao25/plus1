    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-4 top-4 z-20 flex items-start justify-between gap-3">
        <div className="glass-overlay max-w-[72%] rounded-full border px-3 py-2">
          <p className="flex items-center gap-1.5 truncate text-[0.72rem] font-extrabold uppercase tracking-[0.12em] text-white">
        ...
        <div className="glass-overlay rounded-full border px-3 py-2">
          <p className="text-[0.72rem] font-extrabold uppercase tracking-[0.12em] text-white">
            {quest.category}
          </p>
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute inset-x-3.5 bottom-3.5 z-20">
        <div className="glass-overlay event-card-deck rounded-[1.45rem] border p-4">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-white/68">Host</p>
            </div>
          </div>

          <h2 className="mt-4 line-clamp-2 text-[2rem] font-black leading-[0.95] tracking-normal text-white [text-shadow:0_4px_18px_rgba(0,0,0,0.74)]">
            {quest.title}
          </h2>
          ...
          <button
            ...
            className={`pointer-events-auto mt-4 min-h-12 w-full rounded-full px-5 py-3 text-[0.95rem] font-black transition active:scale-[0.98] ${
              actionState.isPrimary
                ? "glass-action border text-zinc-950 hover:bg-white/90"
                : "border border-white/14 bg-white/12 text-white/58"
            }`}
          >
            {actionState.label}
          </button>
...
function Fact({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <div className="flex min-h-10 min-w-0 items-center gap-2 rounded-2xl border border-white/12 bg-white/10 px-3 py-2">
      <span className="shrink-0 text-white/78">{icon}</span>
      <span className="truncate text-xs font-bold text-white/86">{label}</span>
    </div>
  );
}
