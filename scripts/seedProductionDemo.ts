...
type DemoEvent = {
  ...
  hostSlug: string;
  title: string;
  category: "Food" | "Study" | "Fitness" | "Outdoors" | "Social" | "Other";
  location: string;
...
const demoEventSeeds: DemoEventSeed[] = [
  ...
  ["thrift-loop", "ty", "Menlo thrift loop", "Other", "Menlo Park", 28, "Looking for jackets, weird lamps, and dorm room upgrades.", 6],
  ...
  ["ceramics-dropin", "lorde", "Ceramics drop-in", "Other", "Roble Arts Gym", 34, "Making tiny imperfect bowls. No experience needed.", 5],
  ...
  ["record-store", "billie", "Record store crawl", "Other", "The Record Man", 70, "Digging through bins and making each other buy one wildcard.", 5],
...
async function ensureDemoEventsAndSocialGraph(
  ...
  await checked(supabase.from("quests").upsert(events), "upsert demo events");
